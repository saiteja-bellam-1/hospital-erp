...
### Installation (Inno Setup wizard, not a React wizard)
- The React `SetupWizard` and `/api/setup/*` routes have been removed. Old docs that mention `backend/app/routes/setup.py` are stale.
- The only first-install path on Windows: `installer/installer.iss` → operator answers data-folder mode + hospital + admin + optional `.lic` → wizard writes `{app}\data\install_seed.json` (+ `.install_seed.pwd` for fresh mode, ACL'd to SYSTEM + Administrators).
- `backend/launcher.py` calls `app.services.bootstrap_from_seed.consume_seed_if_present(exe_dir)` on first launch. Three modes:
  - `fresh` → `app.services.db_seed.init_database_and_seed` creates hospital + super_admin + roles + modules; `store_license` applies the .lic if present.
  - `adopt_existing` → rebinds `config.json` + the engine to an existing data folder.
  - `restore_backup` → SQLite `.backup()` from a chosen `.db` into the target folder, then runs `migrate_patient_fields` to bring schema forward.
- The wizard's pre-install checks (machine-id, writable folder, DB validity, license validity, backup-DB validity) run via `installer/bin/dbcheck.exe` — a PyInstaller-built CLI (`installer/dbcheck/dbcheck.py`) that emits JSON on stdout. Its spec deliberately excludes `sqlalchemy`/`fastapi`/`uvicorn`, so anything the CLI imports from the backend must avoid those at module import time. `inspect_license_file` currently lives in `app/services/license_service.py` whose top-level `from sqlalchemy.orm import Session` breaks `dbcheck validate-license` with "No module named sqlalchemy" — move the pure verification helpers to a Session-free module before reusing them from dbcheck.
- Running `python main.py` from source with no `config.json` does **not** trigger any seed path. Hand-roll an `install_seed.json` in `data/` or call `init_database_and_seed` directly to bootstrap a dev DB.
- After install, hospital admin can upload/replace `.lic` via Dashboard → License Management.
