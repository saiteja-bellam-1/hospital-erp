...
def _seed_final_bill(db_session, seed_data, *, bill_date, pharmacy=300, food=150, room=300):
    ...
    bill = Bill(
        ...
        bill_subtype="final",
        reference_id=555001,
        subtotal=pharmacy + food + room,
        total_amount=pharmacy + food + room,
        status="paid",
        bill_date=bill_date,
...
def test_settlement_config_and_summary_preview(client, auth_headers, seed_data, db_session):
    ...
    resp = client.put("/api/hospital/settlement-config", json={"pharmacy": 60}, headers=auth_headers)
    assert resp.status_code == 200, resp.text
    assert resp.json()["config"]["pharmacy"] == 60

    summary = client.get(
        "/api/hospital/inpatient-settlements-summary?from=2002-03-10&to=2002-03-10",
        headers=auth_headers,
    ).json()
    pharmacy_unit = next(u for u in summary["units"] if u["unit"] == "pharmacy")
    assert pharmacy_unit["gross_amount"] == 300.0
    assert pharmacy_unit["payout_percentage"] == 60
    assert pharmacy_unit["payout_amount"] == 180.0
    assert pharmacy_unit["hospital_share"] == 120.0
...
def test_record_settlement_and_overlap_guard(client, auth_headers, seed_data, db_session):
    ...
    # Unknown / removed units are rejected.
    other = client.post(
        "/api/hospital/settlements",
...
def test_record_settlement_rejects_zero_revenue(client, auth_headers, seed_data):
    resp = client.post(
        "/api/hospital/settlements",
        json={"unit": "pharmacy", "from": "1990-01-01", "to": "1990-01-31"},
        headers=auth_headers,
    )
